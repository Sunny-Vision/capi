...
@Secured("UF1504")
@FuncCode("UF1504")
@Controller("AssignmentTransferInOutApprovalController")
@RequestMapping("assignmentAllocationAndReallocation/AssignmentTransferInOutApproval")
public class AssignmentTransferInOutApprovalController {

	private static final Logger logger = LoggerFactory.getLogger(AssignmentTransferInOutApprovalController.class);
	...
	public @ResponseBody DatatableResponseModel<TableList>
		...
		try {
			...
			List<Integer> submitToApproveIds = new ArrayList<Integer>();
			if (!(((detail.getAuthorityLevel() & 256) == 256) || ((detail.getAuthorityLevel() & 2048) == 2048))) {
				// not (256): Business Data Administrator, (2048): Business Data Viewer
				submitToApproveIds.add(detail.getUserId());
				submitToApproveIds.addAll(detail.getActedUsers());
			}
			return service.getTableListForApproval(requestModel, referenceMonth, fromUserId, submitToApproveIds, new String[]{"Recommended"});
		} catch (Exception e) {
	...
	public String edit(Model model, @RequestParam(value = "id", required = true) Integer id,
		...
		try {
			...
			if (viewModel == null) {
				...
				return "redirect:/assignmentAllocationAndReallocation/AssignmentTransferInOutApproval/home";
			}
	...
	public @ResponseBody DatatableResponseModel<RecommendAssignmentModel>
		...
		try {
			return service.getRecommendAssignmentTableList(requestModel, assignmentAdjustmentId,
					tpuId, outletTypeId, districtId, batchId, false);
		} catch (Exception e) {
			logger.error("query", e);
		}
		return null;
