...
@Secured("RF9040")
@FuncCode("RF9040")
@Controller("ApplicationTimeOffController")
@RequestMapping("report/ApplicationTimeOff")
public class ApplicationTimeOffController  extends ReportBaseController{

	private static final Logger logger = LoggerFactory.getLogger(ApplicationTimeOffController.class);
	
	@Resource(name="messageSource")
	MessageSource messageSource;
	
	@Autowired
	@Qualifier( "ApplicationTimeOffService")
	...
	public ModelAndView downloadFile(Integer id, ModelAndView mav, RedirectAttributes redirectAttributes, Locale locale, HttpServletResponse response){
		...
		return new ModelAndView("redirect:/report/ApplicationTimeOff/home");
	}
	...
	public String submitReportRequest(ApplicationOTimeOffWorkCriteria criteria, Authentication auth, RedirectAttributes redirectAttributes, Locale locale){
		...
		return "redirect:/report/ApplicationTimeOff/home";
	}


	@RequestMapping(value = "getStaffTimeOffDates")
	public @ResponseBody List<String> getStaffTimeOffDates(Integer userId){		
		try{
			return calendarEventService.getTimeOffDatesInYear(userId);
		}
		catch(Exception ex){
