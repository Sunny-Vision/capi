...
@Secured("RF9010")
@FuncCode("RF9010")
@Controller("AirportTicketPRListByCountryWithCPISeriesController")
@RequestMapping("report/AirportTicketPRListByCountryWithCPISeries")
public class AirportTicketPRListByCountryWithCPISeriesController  extends ReportBaseController{

	private static final Logger logger = LoggerFactory.getLogger(AirportTicketPRListByCountryWithCPISeriesController.class);
	
	@Autowired
	@Qualifier( "AirportTicketPRListByCountryWithCPISeriesService")
	...
	public ModelAndView downloadFile(Integer id, ModelAndView mav, RedirectAttributes redirectAttributes, Locale locale, HttpServletResponse response){
		...
		return new ModelAndView("redirect:/report/AirportTicketPRListByCountryWithCPISeries/home");
	}
	 ...
	 */
	@RequestMapping(value = "queryCpiCompilationSeriesSelect2", method = RequestMethod.GET)
	public @ResponseBody Select2ResponseModel queryCpiCompilationSeriesSelect2(Select2RequestModel requestModel) {
		try {
			return quotationService.queryCpiCompilationSeriesSelect2(requestModel);
		} catch (Exception e) {
			logger.error("queryCpiCompilationSeriesSelect2", e);
		}
		return null;
	}
	...
	public String submitReportRequest(AirportTicketPRListByCountryWithCPISeriesCriteria criteria, Integer taskType, Authentication auth, RedirectAttributes redirectAttributes, Locale locale){
		CapiWebAuthenticationDetails detail = (CapiWebAuthenticationDetails)auth.getDetails();
		Integer userId = detail.getUserId();
		this.generateReport(criteria, userId, redirectAttributes, locale, taskType);
		return "redirect:/report/AirportTicketPRListByCountryWithCPISeries/home";
	}
