...
@Secured("RF9008")
@FuncCode("RF9008")
@Controller("NewRecruitmentsAndProductReplacementsController")
@RequestMapping("report/NewRecruitmentsAndProductReplacements")
public class NewRecruitmentsAndProductReplacementsController  extends ReportBaseController{

	private static final Logger logger = LoggerFactory.getLogger(NewRecruitmentsAndProductReplacementsController.class);

	@Autowired
	@Qualifier( "NewRecruitmentsAndProductReplacementsService")
	...
	public ModelAndView downloadFile(Integer id, ModelAndView mav, RedirectAttributes redirectAttributes, Locale locale, HttpServletResponse response){
		...
		return new ModelAndView("redirect:/report/NewRecruitmentsAndProductReplacements/home");
	}
	...
	public String submitReportRequest(NewRecruitmentsAndProductReplacementsCriteria criteria, 
			Integer taskType, Authentication auth, RedirectAttributes redirectAttributes, Locale locale){
		CapiWebAuthenticationDetails detail = (CapiWebAuthenticationDetails)auth.getDetails();
		Integer userId = detail.getUserId();
		this.generateReport(criteria, userId, redirectAttributes, locale, taskType);	
		return "redirect:/report/NewRecruitmentsAndProductReplacements/home";
	}
