...
@Secured("RF9035")
@FuncCode("RF9035")
@Controller("IndividualQuotationRecordController")
@RequestMapping("report/IndividualQuotationRecord")
public class IndividualQuotationRecordController  extends ReportBaseController{

	private static final Logger logger = LoggerFactory.getLogger(IndividualQuotationRecordController.class);
	
	@Autowired
	@Qualifier( "IndividualQuotationRecordService")
	...
	public ModelAndView downloadFile(Integer id, ModelAndView mav, RedirectAttributes redirectAttributes, Locale locale, HttpServletResponse response){
		...
		return new ModelAndView("redirect:/report/IndividualQuotationRecord/home");
	}
	

	@RequestMapping(value = "submitReportRequest")
	public String submitReportRequest(IndividualQuotationRecordCriteria criteria, Integer taskType, Authentication auth, 
			RedirectAttributes redirectAttributes, Locale locale, HttpServletRequest req){
		CapiWebAuthenticationDetails detail = (CapiWebAuthenticationDetails)auth.getDetails();
		Integer userId = detail.getUserId();	
		criteria.setContextPath(req.getContextPath());
		if(criteria.getIsgetImage() == null || criteria.getIsgetImage() == false){
			criteria.setIsgetImage(false);
		}
		this.generateReport(criteria, userId, redirectAttributes, locale, taskType);		
		return "redirect:/report/IndividualQuotationRecord/home";
	}
	...
	@RequestMapping(value = "queryAssignmentSelect2", method = RequestMethod.GET)
	public @ResponseBody Select2ResponseModel queryAssignmentSelect2(Select2RequestModel requestModel,
			@RequestParam(value = "referenceMonth", required = false) String referenceMonth){
		try {
			return assignmentService.queryAssignmentSelect2(requestModel, referenceMonth);
		} catch (Exception e) {
			logger.error("queryUnitSelect2", e);
		}
		return null;
	...
	public @ResponseBody Select2ResponseModel queryQuotationSelect2(Select2RequestModel requestModel,
		...
		try {
			return quotationService.queryQuotationSelectAssignmentIds2(requestModel, purposeId, unitId, assignmentIds);
		} catch (Exception e) {
	 ...
	 * Get purpose select format
	 */
	@RequestMapping(value = "queryQuotationRecordSelect2", method = RequestMethod.GET)
	public @ResponseBody Select2ResponseModel queryQuotationRecordSelect2(Select2RequestModel requestModel,
			@RequestParam(value = "quotationId[]", required = false) Integer[] quotationId,
			@RequestParam(value = "assignmentIds[]", required = false) Integer[] assignmentIds,
			@RequestParam(value = "referenceMonth", required = false) String referenceMonth) {
		try {
			return quotationRecordService.queryQuotationRecordSelect2(requestModel, quotationId, assignmentIds, referenceMonth);
		} catch (Exception e) {
			logger.error("queryPurposeSelect2", e);
		}
		return null;
