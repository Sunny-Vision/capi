...
@Secured("RF9022")
@FuncCode("RF9022")
@Controller("QuotationStatisticsReportByVarietyController")
@RequestMapping("report/QuotationStatisticsReportByVariety")
public class QuotationStatisticsReportByVarietyController  extends ReportBaseController{

	private static final Logger logger = LoggerFactory.getLogger(QuotationStatisticsReportByVarietyController.class);
	
	@Autowired
	@Qualifier( "QuotationStatisticsReportByVarietyService")
	...
	public ModelAndView downloadFile(Integer id, ModelAndView mav, RedirectAttributes redirectAttributes, Locale locale, HttpServletResponse response){
		...
		return new ModelAndView("redirect:/report/QuotationStatisticsReportByVariety/home");
	}
	...
	public @ResponseBody Select2ResponseModel queryQuotationSelect2(Select2RequestModel requestModel,
			...
			@RequestParam(value = "cpiSurveyForm[]", required = false) Integer[] cpiSurveyForm) {
		try {
			return quotationService.queryQuotationSelectWithcpiSurveyForm2(requestModel, purposeId, unitId, cpiSurveyForm);
		} catch (Exception e) {
	...
	public String submitReportRequest(QuotationStatisticsReportByVarietyCriteria criteria, Integer taskType, Authentication auth, RedirectAttributes redirectAttributes, Locale locale){
		CapiWebAuthenticationDetails detail = (CapiWebAuthenticationDetails)auth.getDetails();
		Integer userId = detail.getUserId();
		this.generateReport(criteria, userId, redirectAttributes, locale, taskType);
		return "redirect:/report/QuotationStatisticsReportByVariety/home";
	}
