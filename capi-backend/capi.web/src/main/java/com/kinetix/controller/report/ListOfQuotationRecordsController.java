...
@Secured("RF9007")
@FuncCode("RF9007")
@Controller("ListOfQuotationRecordsController")
@RequestMapping("report/ListOfQuotationRecords")
public class ListOfQuotationRecordsController  extends ReportBaseController{

	private static final Logger logger = LoggerFactory.getLogger(ListOfQuotationRecordsController.class);

	@Autowired
	@Qualifier( "ListOfQuotationRecordsService")
	...
	public ModelAndView downloadFile(Integer id, ModelAndView mav, RedirectAttributes redirectAttributes, Locale locale, HttpServletResponse response){
		...
		return new ModelAndView("redirect:/report/ListOfQuotationRecords/home");
	}
	...
	public String submitReportRequest(ListOfQuotationRecordsCriteria criteria, Authentication auth, RedirectAttributes redirectAttributes, Locale locale){
		CapiWebAuthenticationDetails detail = (CapiWebAuthenticationDetails)auth.getDetails();
		Integer userId = detail.getUserId();
		this.generateReport(criteria, userId, redirectAttributes, locale, 2);	
		return "redirect:/report/ListOfQuotationRecords/home";
	}
