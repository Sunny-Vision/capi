...
<div class="box-body quotation-record-list-container">
			...
			<c:if test="${model.normalAssignment}">
				...
				<div class="col-md-2">
					<select name="dateSelectedAssignmentId" class="form-control filters select2ajax"
						...
						data-ajax-url="<c:url value='/assignmentManagement/AssignmentMaintenance/queryDateSelectionForIPSelect2'/>?userId=${model.userId}&outletId=${model.outletId}"></select>
				</div>
			</c:if>
			<div class="consignment-counter-container hide">
				...
				<div class="col-md-2">
					<select name="consignmentCounter" class="form-control filters select2ajax"
						...
						data-ajax-url="<c:url value='/assignmentManagement/AssignmentMaintenance/queryIPConsignmentSelect2'/>?userId=${model.userId}"></select>
				</div>
			</div>
			<div class="unit-category-container hide">
				...
				<div class="col-md-2">
					<select name="unitCategory" class="form-control filters select2ajax"
						data-allow-clear="true"
						data-ajax-url="<c:url value='/assignmentManagement/AssignmentMaintenance/queryIPDistinctUnitCategorySelect2'/>?userId=${model.userId}"></select>
				</div>
			</div>
		</div>
	...
	<table class="table table-striped table-bordered table-hover quotation-record-table"
		data-url="<c:url value="/assignmentManagement/AssignmentMaintenance/queryIPQuotationRecord"/>">
		<thead>
		<tr>
			<th>Form Type</th>
			<th>IsICP</th>
