...
<t:layout>
	...
	<jsp:attribute name="header">
		...
		<%@include file="/WEB-INF/views/assignmentManagement/RUACaseApproval/assignmentUnitCategoryInfoDialog-js.jsp"%>
			...
			$(function() {
				...
				function initTabPane($tabPane) {
					...
					$dataTable.on('click', '.btn-redirect', function(e) {
						...
						$.post('<c:url value="/assignmentManagement/RUACaseApproval/cacheQuotationRecordSearchFilterAndResult"/>',
							sessionObj,
							function() {
								location = '<c:url value="/assignmentManagement/RUACaseApproval/editQuotationRecord"/>?quotationRecordId=' + id;
							});
					});
					...
					$dataTable.on('click', '.btn-small-submit', function() {
						...
						$.post('<c:url value="/assignmentManagement/RUACaseApproval/cacheQuotationRecordSearchFilterAndResult"/>',
							sessionObj,
							function() {
								location = '<c:url value="/assignmentManagement/RUACaseApproval/smallSubmit"/>?quotationRecordId=' + id;
							});
					})
					...
					$('.assignmentUnitCategoryInfoDialog', $tabPane).assignmentUnitCategoryInfoDialog({
						getContentUrl: function() {
							if (tabName == 'Normal')
								return '<c:url value='/assignmentManagement/RUACaseApproval/assignmentUnitCategoryInfoDialogNormalContent'/>';
						},
							...
							$('[name="dateSelectedAssignmentId"]', $tabPane).change(function() {
								if ($(this).val() != null) {
									...
									if (dateSelectedAssignmentId != null) {
										$.get('<c:url value='/assignmentManagement/RUACaseApproval/getStatusByAssignment'/>', 
					...
					$('.btn-set-visited', $tabPane).click(function() {
						...
						normalFormPost('<c:url value="/assignmentManagement/RUACaseApproval/setVisited"/>', params);
					});
					...
					$('.btn-update-time-log', $tabPane).click(function() {
						...
						if ($timeLogDatePickerDialogModal == null) {
							...
							$('.modal-submit', $timeLogDatePickerDialogModal).click(function() {
								...
								var checkUrl = '<c:url value='/assignmentManagement/RUACaseApproval/checkTimeLogExists'/>';
        		...
        		<div class="col-md-12">
        			...
        			<div class="box box-primary ${empty model.pointToNote ? "collapsed-box" : ""}">
        				<div class="box-header with-border">
							<a class="btn btn-default" href="<c:url value='/assignmentManagement/RUACaseApproval/home'/>">Back</a>
							<c:choose>
					...
					<div class="nav-tabs-custom">
						...
						<div class="tab-content" style="padding: 10px 0 0 0">
							<div id="tab_Normal" class="tab-pane">
								<%@include file="/WEB-INF/views/assignmentManagement/RUACaseApproval/editQuotationRecordListNormal.jsp"%>
							</div>
							<!-- /.tab-pane -->
						</div>
						<!-- /.tab-content -->
