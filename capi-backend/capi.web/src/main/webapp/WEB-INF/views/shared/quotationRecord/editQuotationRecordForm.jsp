...
<div class="form-horizontal quotation-record-form ${quotationRecord.backNo ? 'back-no' : 'original active'}">
	...
	<div class="form-group ${quotationRecord.NPriceMandatory ? 'required' : ''}">
		<div class="btn-copy btn-copy-nprice copy-nprice ${model.history ? '' : 'hide'}"><i class="fa fa-copy"></i></div>
		<label class="col-md-4 control-label">Final N price</label>
		<div class="col-md-3">
			<input
				name="${model.readonly ? 'readonly.' : ''}${quotationRecord.backNo ? 'backNoQuotationRecord.' : 'quotationRecord.'}nPrice" type="text" class="form-control nPrice" value="<c:out value="${quotationRecord.nPrice}" />"
	...
	<div class="form-group ${quotationRecord.SPriceMandatory ? 'required' : ''}">
		<div class="btn-copy btn-copy-sprice copy-sprice ${model.history ? '' : 'hide'}"><i class="fa fa-copy"></i></div>
		<label class="col-md-4 control-label">Final S price</label>
		<div class="col-md-3">
			<input
				name="${model.readonly ? 'readonly.' : ''}${quotationRecord.backNo ? 'backNoQuotationRecord.' : 'quotationRecord.'}sPrice" type="text" class="form-control sPrice" value="<c:out value="${quotationRecord.sPrice}" />"
				data-rule-number="true"
				${quotationRecord.SPricePeculiar or not empty quotationRecord.subPrices or not empty quotationRecord.discount ? 'readonly' : ''} ${model.readonly ? 'disabled': ''}/>
		</div>
		<c:if test="${not model.history}">
		...
		<div class="btn-copy copy-SPricePeculiar hide"><i class="fa fa-copy"></i></div>
		<div class="col-md-3">
			<div class="checkbox">
				<label>
					<input
	...
	<div class="form-group">
		<div class="btn-copy copy-uom ${model.history ? '' : 'hide'}"><i class="fa fa-copy"></i></div>
	...
	<div class="form-group">
		<div class="btn-copy copy-discount ${model.history ? '' : 'hide'}"><i class="fa fa-copy"></i></div>
		<label class="col-md-4 control-label">Sub-Price</label>
		<div class="col-md-2">
			<p class="form-control-static">
	...
	<c:if test="${!quotationRecord.backNo && originalQuotationRecord.FRRequired }">
		<c:if test="${originalQuotationRecord.collectFR}">
			<div class="form-group frFields" <c:if test="${originalQuotationRecord.useFRAdmin && originalQuotationRecord.collectFR && !quotationRecord.consignmentCounter }">style="display:none"</c:if>>
				<div class="btn-copy copy-fr ${model.history ? '' : 'hide'}"><i class="fa fa-copy"></i></div>
		...
		<c:if test="${(originalQuotationRecord.collectFR && !originalQuotationRecord.useFRAdmin) || originalQuotationRecord.useFRAdmin}">
			<div class="form-group">
				<div class="btn-copy copy-consignment-counter ${model.history ? '' : 'hide'}"><i class="fa fa-copy"></i></div>
			...
			<div class="form-group consignmentCounterRemark-container">
				<div class="btn-copy copy-name-value ${model.history ? '' : 'hide'}"><i class="fa fa-copy"></i></div>
	...
	<div class="form-group">
		<div class="btn-copy copy-availability ${model.history ? '' : 'hide'}"><i class="fa fa-copy"></i></div>
...
<%-- 				</c:if> --%>
				...
				<option value="6" ${(quotationRecord.seasonality == 2 || quotationRecord.seasonality == 3) ? "" : "disabled"} 
					${quotationRecord.availability == 6 ? "selected" : ""}>回倉</option>
				<option value="8" ${(quotationRecord.seasonality == 2 || quotationRecord.seasonality == 3) ? "" : "disabled"}
	...
	<c:if test="${not quotationRecord.backNo}">
		<div class="form-group">
			...
			<div class="col-md-8">
				<c:choose>
				<c:when test="${not model.readonly}">
					<input name="${model.readonly ? 'readonly.' : ''}${quotationRecord.backNo ? 'backNoQuotationRecord.' : 'quotationRecord.'}productRemark" type="text" class="form-control" value="<c:out value="${quotationRecord.productRemark}" />" />
				</c:when>
				<c:otherwise>
					<p class="form-control-static break-text" name="${quotationRecord.backNo ? 'backNoQuotationRecord.' : 'quotationRecord.'}productRemark"><c:out value="${quotationRecord.productRemark}" /></p>
				</c:otherwise>
				</c:choose>
			</div>
		</div>
		<div class="form-group">
			...
			<div class="col-md-8">
				<c:choose>
				<c:when test="${not model.readonly}">
					<input name="${model.readonly ? 'readonly.' : ''}${quotationRecord.backNo ? 'backNoQuotationRecord.' : 'quotationRecord.'}productPosition" type="text" class="form-control" value="<c:out value="${quotationRecord.productPosition}" />" />
				</c:when>
				<c:otherwise>
					<p class="form-control-static break-text" name="${quotationRecord.backNo ? 'backNoQuotationRecord.' : 'quotationRecord.'}productPosition"><c:out value="${quotationRecord.productPosition}" /></p>
				</c:otherwise>
				</c:choose>
			</div>
		</div>
	...
	<div class="form-group">
		...
		<div class="col-md-8">
			<p class="form-control-static break-text">${quotationRecord.assignmentContactPerson}</p>
		</div>
	</div>
	<div class="form-group">
