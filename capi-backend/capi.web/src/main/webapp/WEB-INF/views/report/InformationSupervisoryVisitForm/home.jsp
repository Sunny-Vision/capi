...
<t:layout>
		...
		<script>
			$(document).ready(function(){
				...
				$dataTable.DataTable({
					...
					"processing": true,
	                "serverSide": true,
	                "ajax": "<c:url value='/report/InformationSupervisoryVisitForm/query'/>",
	                "columns": [
	                            ...
	                            },
	                            {
	                            	...
	                            	"render" : function(data, type, full, meta){
	                            		if (data == "Finished"){
	                            			return "<a href='<c:url value='/report/InformationSupervisoryVisitForm/downloadFile?id='/>"+full.reportTaskId+"' class='table-btn'><span class='glyphicon glyphicon-download-alt' aria-hidden='true'></span></a>";
	                            		}
	                            		else{
	                            			return data;
	                            		}
				...
				$('.month-picker').datepicker({
					...
					$.get("<c:url value='/report/InformationSupervisoryVisitForm/getSVDates' />", {date: $('.month-picker').val()}, function (data){
						svDates = [];
						for (i = 0; i < data.length; i++){
							svDates.push(parseDate(data[i]));
						}
				...
				Datepicker(".date-picker",{
					...
					beforeShowDay: function(date) {
	            		for (var i = 0; i < svDates.length; i++) {
	            			var str_date = svDates[i];
	...
	<jsp:body>
		<section class="content-header">
          <h1>Report of information of Supervisory Visit forms</h1>
        </section>
				...
				<!-- content -->
					<div class="box" >
						<div class="box-body">
							...
							<div id="filters">
								<form class="form-horizontal" action="<c:url value="/report/InformationSupervisoryVisitForm/submitReportRequest" />" id="criteriaForm" method="post">
											...
											<div class="col-md-5">
												<div class="input-group">
													...
													<select multiple name="fieldOfficerId" id="userId" class="select2ajax filters"
														...
														data-placeholder=""
														data-ajax-url="<c:url value='/report/InformationSupervisoryVisitForm/queryOfficerSelect2'/>"
														data-get-key-value-by-ids-url="<c:url value='/report/InformationSupervisoryVisitForm/queryUserSelectMutiple'/>"
														required
													></select>
										...
										<div class="form-group">
											...
											<div class="col-md-2">
												<div class="input-group date date-picker" data-orientation="top" data-multidate="true"
													 data-target="[name=dateDisplay]" data-input="#supervisoryVisitDates" >
													<input type="text" class="form-control" style="display: none;" id="supervisoryVisitDates" name="supervisoryVisitDates" value="" required readonly />
			       									<select class="form-control select2" disabled name="dateDisplay" multiple style="display:none" ></select>
													<div id="supervisoryVisitDatesIcon" class="input-group-addon">
														<i class="fa fa-calendar"></i>
													</div>
												</div>
											</div>
