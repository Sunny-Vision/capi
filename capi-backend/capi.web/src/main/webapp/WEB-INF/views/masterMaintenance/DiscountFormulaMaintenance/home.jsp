		...
		<script>
			function deleteRecordsWithConfirm(data) {
				bootbox.confirm({
					...
					callback: function(result){
						if (result){
							$.post("<c:url value='/masterMaintenance/DiscountFormulaMaintenance/delete'/>",
								data,
								function(response) {
			...
			$(document).ready(function(){
				...
				$dataTable.DataTable({
					...
					"buttons": [
						{
							...
							"action": function( nButton, oConfig, flash ) {
								window.location = "<c:url value='/masterMaintenance/DiscountFormulaMaintenance/edit'/>";
							}
						},
						{
							"text": "Delete",
					...
					"processing": true,
	                "serverSide": true,
	                //"ajax": "<c:url value='/masterMaintenance/DiscountFormulaMaintenance/query'/>",
	                "ajax": {
	                	url: "<c:url value='/masterMaintenance/DiscountFormulaMaintenance/query'/>",
	                	method: "post"
	                },
	                "columns": [
	                            {
	                            ...
	                            },
	                            {
	                            	...
	                            	"searchable": false,
	                            	"render" : function(data, type, full, meta){
	                            		var html = "<a href='<c:url value='/masterMaintenance/DiscountFormulaMaintenance/edit?id='/>"+data+"' class='table-btn'><span class='glyphicon glyphicon-pencil' aria-hidden='true'></span></a>";
	                            		<sec:authorize access="hasPermission(#user, 256)">
							...
							<div class="box-body">
								...
								<table class="table table-striped table-bordered table-hover" id="dataList">
									...
									<tr>
										...
										<th>Discount Pattern</th>
										<th>Discount Formula</th>
										<th>Status</th>
