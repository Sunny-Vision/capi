		...
		<script>
			function deleteRecordsWithConfirm(data) {
				bootbox.confirm({
					...
					callback: function(result){
						if (result){
							$.post("<c:url value='/masterMaintenance/BatchMaintenance/delete'/>",
								data,
								function(response) {
			...
			$(document).ready(function(){
				...
				$dataTable.DataTable({
					...
					"buttons": [
						{
							...
							"action": function( nButton, oConfig, flash ) {
								window.location = "<c:url value='/masterMaintenance/BatchMaintenance/edit?act=add'/>";
							}
						},
						{
							"text": "Delete",
	                ...
	                "serverSide": true,
	                "ajax": {
	                	url: "<c:url value='/masterMaintenance/BatchMaintenance/query'/>",
	                	method: "post",
	                	data: function(d) {
	                		d.surveyForm = $('#surveyForm').val();
	                	}
	                },
	                "columns": [
								{
	                            ...
	                            },
	                            {
	                            	...
	                            	"searchable": false,
	                            	"render" : function(data, type, full, meta){
	                            		var html = "<a href='<c:url value='/masterMaintenance/BatchMaintenance/edit?act=edit&id='/>"+data+"' class='table-btn'><span class='glyphicon glyphicon-pencil' aria-hidden='true'></span></a>";
	                            		<sec:authorize access="hasPermission(#user, 256)">
	...
	<jsp:body>
        ...
        <section class="content">
        	<div class="row">
				<div class="col-md-12" >
				<!-- content -->
					<form action="<c:url value="/masterMaintenance/BatchMaintenance/delete" />" id="dataForm" method="post">
						...
						<div class="box" >
							<div class="box-body">
								...
								<div class="form-horizontal">
									<div class="row">
										<label class="control-label col-md-1">Survey Form</label>
										<div class="col-md-2">
											<select class="form-control select2" id="surveyForm" 
												data-ajax-url="<c:url value='/masterMaintenance/BatchMaintenance/querySurveyFormSelect'/>"></select>
										</div>
									</div>
								</div>
								<hr/>
