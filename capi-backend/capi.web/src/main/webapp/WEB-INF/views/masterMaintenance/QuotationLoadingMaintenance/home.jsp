		...
		function deleteRecordsWithConfirm(data) {
			bootbox.confirm({
				...
				callback: function(result){
					if (result){
						$.post("<c:url value='/masterMaintenance/QuotationLoadingMaintenance/delete'/>",
							data,
							function(response) {
			...
			$(document).ready(function(){
				...
				$dataTable.DataTable({
					...
					"buttons": [
						{
							...
							"action": function( nButton, oConfig, flash ) {
								window.location = "<c:url value='/masterMaintenance/QuotationLoadingMaintenance/edit?act=add'/>";
							}
						},
						{
							"text": "Delete",
					...
					"processing": true,
	                "serverSide": true,
	                //"ajax": "<c:url value='/masterMaintenance/QuotationLoadingMaintenance/query'/>",
	                "ajax": {
	                	url: "<c:url value='/masterMaintenance/QuotationLoadingMaintenance/query'/>",
	                	method: "post"
	                },
	                "columns": [
								{
	                            ...
	                            },
	                            {
	                            	...
	                            	"searchable": false,
	                            	"render" : function(data, type, full, meta){
	                            		var html = "<a href='<c:url value='/masterMaintenance/QuotationLoadingMaintenance/edit'/>?act=edit&id="+data+"&outletType="+full.outletType+"' class='table-btn'><span class='glyphicon glyphicon-pencil' aria-hidden='true'></span></a>";
	                            		<sec:authorize access="hasPermission(#user, 256)">
						...
						<div class="box-body">
							...
							<table class="table table-striped table-bordered table-hover" id="dataList">
								...
								<tr>
									...
									<th>District</th>
									<th>Outlet Types</th>
									<th>No. of Quotations per Man-Day</th>
