		...
		function deleteRecordsWithConfirm(data) {
			bootbox.confirm({
				...
				callback: function(result){
					if (result){
						$.post("<c:url value='/masterMaintenance/UOMConversionMaintenance/delete'/>",
							data,
							function(response) {
			...
			*/
			...
			$(document).ready(function(){
				...
				$dataTable.DataTable({
					...
					"buttons": [
						{
							...
							"action": function( nButton, oConfig, flash ) {
								window.location = "<c:url value='/masterMaintenance/UOMConversionMaintenance/edit?act=add'/>";
							}
						},
						{
							"text": "Delete",
					...
					"processing": true,
	                "serverSide": true,
	                //"ajax": "<c:url value='/masterMaintenance/UOMConversionMaintenance/query'/>",
	                "ajax": {
	                	url: "<c:url value='/masterMaintenance/UOMConversionMaintenance/query'/>",
	                	method: "post"
	                },
	                "columns": [
								{
	                            ...
	                            },
	                            {
	                            	...
	                            	"searchable": false,
	                            	"render" : function(data, type, full, meta){
	                            		var html = "<a href='<c:url value='/masterMaintenance/UOMConversionMaintenance/edit?act=edit&id='/>"+data+"' class='table-btn'><span class='glyphicon glyphicon-pencil' aria-hidden='true'></span></a>";
	                            		<sec:authorize access="hasPermission(#user, 256)">
