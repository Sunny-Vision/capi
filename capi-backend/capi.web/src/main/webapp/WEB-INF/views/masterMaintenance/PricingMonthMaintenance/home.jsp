		...
		<script>
			function deleteRecordsWithConfirm(data) {
				bootbox.confirm({
					...
					callback: function(result){
						if (result){
							$.post("<c:url value='/masterMaintenance/PricingMonthMaintenance/delete'/>",
								data,
								function(response) {
			...
			$(document).ready(function(){
				...
				$dataTable.DataTable({
					...
					"buttons": [
						{
							...
							"action": function( nButton, oConfig, flash ) {
								window.location = "<c:url value='/masterMaintenance/PricingMonthMaintenance/edit?act=add'/>";
							}
						},
						{
							"text": "Delete",
					...
					"processing": true,
	                "serverSide": true,
	                //"ajax": "<c:url value='/masterMaintenance/PricingMonthMaintenance/query'/>",
	                "ajax": {
	                	url: "<c:url value='/masterMaintenance/PricingMonthMaintenance/query'/>",
	                	method: "post"
	                },
	                "columns": [
								{
									"data": "pricingFrequencyId",
									"orderable": false,
									"searchable": false,
	                            ...
	                            },
	                            {
	                            	...
	                            	"searchable": false,
	                            	"render" : function(data, type, full, meta){
	                            		var html = "<a href='<c:url value='/masterMaintenance/PricingMonthMaintenance/edit?act=edit&id='/>"+data+"' class='table-btn'><span class='glyphicon glyphicon-pencil' aria-hidden='true'></span></a>";
	                            		<sec:authorize access="hasPermission(#user, 256)">
							...
							<div class="box-body">
								...
								<table class="table table-striped table-bordered table-hover" id="dataList">
									...
									<tr>
										...
										<th>Pricing Month ID</th>
										<th>Pricing Month Description</th>
										<th>Months</th>
