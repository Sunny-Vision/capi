		...
		<script>
			function deleteRecordsWithConfirm(data) {
				bootbox.confirm({
					...
					callback: function(result){
						if (result){
							$.post("<c:url value='/masterMaintenance/UomMaintenance/delete'/>",
								data,
								function(response) {
			...
			$(document).ready(function(){		
				...
				$dataTable.DataTable({
					...
					"buttons": [
						{
							...
							"action": function( nButton, oConfig, flash ) {
								window.location = "<c:url value='/masterMaintenance/UomMaintenance/edit?act=add'/>";
							}
						},
						{
							"text": "Delete",
	                ...
	                "serverSide": true,
	                "ajax": {
	                	url: "<c:url value='/masterMaintenance/UomMaintenance/query'/>",
	                	method: "post",
	                	data: function(d) {
	                		d.uomCategoryId = $('#uomCategoryfilter').val();
	                	}
	                },
	                ...
	                "columns": [
	                            ...
	                            },
	                            {
	                            	...
	                            	"searchable": false,
	                            	"render" : function(data, type, full, meta){
	                            		var html = "<a href='<c:url value='/masterMaintenance/UomMaintenance/edit?act=edit&id='/>"+data+"' class='table-btn'><span class='glyphicon glyphicon-pencil' aria-hidden='true'></span></a>";
	                            		<sec:authorize access="hasPermission(#user, 256)">
				...
				$('#uomCategoryfilter').select2ajax({
					placeholder: "Select a UOM Category",
					allowClear: true
				});
				...
				<!-- content -->
						...
						<div class="box" >
							...
							<div class="form-horizontal">
								<div class="row">
									<label class="control-label col-md-1">UOM Category</label>
									<div class="col-md-2">
										<select class="form-control select2 filters" id="uomCategoryfilter" 
										data-ajax-url="<c:url value='/masterMaintenance/UomMaintenance/queryUOMCategorySelect2'/>"></select>
									</div>
								</div>
							</div>
							...
							<hr/>
								<table class="table table-striped table-bordered table-hover" id="dataList">
									...
									<tr>
										...
										<th>Chinese Name</th>
										<th>English Name</th>
										<th>UOM Category Code</th>
										<th>UOM Category Description</th>
