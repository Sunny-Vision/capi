...
(function($, undefined){
				...
				buttons.push({
								...
								"action": function( nButton, oConfig, flash ) {
									var dataModel = {
										...
										team: $('[name="team"]', plugin.$popupModal).val()
									};
			...
			plugin.$popupTable.DataTable({
				...
				"ajax": {
					...
					data: function(dataModel) {
						var team = $('[name="team"]', plugin.$popupModal).val();
	            		if (team != '')
	            			dataModel.team = team;
	...
	$.fn.userLookup = function(option) {
		var pluginDataName = 'reportUserLookup';
	...
	var defaults = $.fn.userLookup.defaults = {
		multiple: true,
		popupUrl: "<c:url value='/commonDialog/ReportUserLookup/home'/>",
		queryUrl: "<c:url value='/commonDialog/ReportUserLookup/query'/>",
		selectAllUrl: "<c:url value='/commonDialog/ReportUserLookup/getLookupTableSelectAll'/>",
		defaultOrder: [[ 1, "asc" ]],
