...
<script>
(function ($) {
	$.fn.assignmentLookupSurveyMonth = function(opts, inputObject) {
		var $input = this;
		var _def = {
			multiple: true,
			popupUrl: "<c:url value='/commonDialog/AssignmentLookup/surveyMonthHome'/>",
			queryUrl: "<c:url value='/commonDialog/AssignmentLookup/surveyMonthQuery'/>",
			selectAllUrl: "<c:url value='/commonDialog/AssignmentLookup/getLookupTableSelectAllSurveyMonth'/>",
			defaultOrder: [[ 1, "asc" ]],
		...
		function initPopup() {
		    ...
		    $popupTable.on('click', '.btn-select-single', function() {
				...
				//inputObject.select2ajax("val", id);
				var singleUrl = inputObject.data('singleUrl');
				inputObject.empty();
				$.ajax({
					url:singleUrl,
					data: { assignmentId: selectedIds },
					method: "post",
					traditional: true,
					success:function(data){
						//var option = new Option(data[0].referenceNo, data[0].assignmentId);
						var option = new Option(data[0].referenceNo, data[0].referenceNo);
						option.selected = true;
						inputObject.append(option);
