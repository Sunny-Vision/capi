...
(function($, undefined){
	...
	Plugin.prototype = {
		...
		_initPopup: function() {
			...
			if (plugin.o.multiple) {
				...
				buttons.push({
								...
								"action": function( nButton, oConfig, flash ) {
									...
									if (plugin.o.ghs != null)
				            			dataModel.ghs = plugin.o.ghs;
				            		else
				            			dataModel.ghs = $('[name="ghs"]', plugin.$popupModal).val();
									
									if (plugin.o.team != null)
				            			dataModel.team = plugin.o.team;
				            		else
				            			dataModel.team = $('[name="team"]', plugin.$popupModal).val();
			...
			if (plugin.o.team == null) {
				$('[name="team"]', plugin.$popupModal).select2ajax({
					allowClear: true,
					placeHolder: '',
					width: "100%"
				});
	...
	$.fn.staffCalendarUserLookup = function(option) {
		var pluginDataName = 'staffCalendarUserLookup';
	...
	$.fn.staffCalendarUserLookup.Constructor = Plugin;

	var defaults = $.fn.staffCalendarUserLookup.defaults = {
		multiple: true,
		popupUrl: "<c:url value='/commonDialog/StaffCalendarUserLookup/home'/>",
		queryUrl: "<c:url value='/commonDialog/StaffCalendarUserLookup/query'/>",
		selectAllUrl: "<c:url value='/commonDialog/StaffCalendarUserLookup/getLookupTableSelectAll'/>",
		defaultOrder: [[ 6, "asc" ]],
