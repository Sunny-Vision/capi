...
(function($, undefined){
	...
	Plugin.prototype = {
		...
		_initPopup: function() {
				...
				buttons.push({
								...
								"action": function( nButton, oConfig, flash ) {
									var dataModel = {
										...
										status: '',//'Active',
										reviewed: $('[name="reviewed"]', plugin.$popupModal).val()
									};
									if (plugin.o.productGroupId != null)
				            			dataModel.productGroupId = plugin.o.productGroupId;
				            		else
				            			dataModel.productGroupId = $('[name="productGroup"]', plugin.$popupModal).val();
			...
			// put product read permission checking here
			...
			if (plugin.o.productGroupId == null) {
				$('.product-group-container').show();
				$('[name="productGroup"]', plugin.$popupModal).select2ajax();
			}
	...
	$.fn.productLookup = function(option) {
		var pluginDataName = 'productLookup';
	...
	$.fn.productLookup.Constructor = Plugin;

	var defaults = $.fn.productLookup.defaults = {
		multiple: true,
		popupUrl: "<c:url value='/commonDialog/ProductLookup/home'/>",
		queryUrl: "<c:url value='/commonDialog/ProductLookup/query'/>",
		selectAllUrl: "<c:url value='/commonDialog/ProductLookup/getLookupTableSelectAll'/>",
		defaultOrder: [[ 1, "asc" ]],
