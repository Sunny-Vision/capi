...
<script>
(function ($) {
	$.fn.peCertaintyCaseLookup = function(opts) {
		var $input = this;
		var _def = {
			multiple: true,
			popupUrl: "<c:url value='/commonDialog/PECertaintyCaseLookup/home'/>",
			queryUrl: "<c:url value='/commonDialog/PECertaintyCaseLookup/query'/>",
			selectAllUrl: "<c:url value='/commonDialog/PECertaintyCaseLookup/getLookupTableSelectAll'/>",
			defaultOrder: [[ 1, "asc" ]],
		...
		function initPopup() {
				...
				buttons.push({
								...
								"action": function( nButton, oConfig, flash ) {
									var dataModel = {
					            		...
					            		tpuId: $('[name="tpuId"]', $popupModal).val(),
					            		
					            		purposeId: $('[name="purposeId"]', $popupModal).val(),
					            		
					            		certaintyCase:$('[name="certaintyCase"]', $popupModal).val()
									};
			...
			$popupTable.DataTable({
	            ...
	            "ajax": {
	            	...
	            	data: function(dataModel) {
	            		...
	            		if (settings.purposeId != null)
	            			dataModel.purposeId = settings.tpuId;
	            		else
	            			dataModel.purposeId = $('[name="purposeId"]', $popupModal).val();
	            		
	            		dataModel.certaintyCase = $('[name="certaintyCase"]', $popupModal).val();
			...
			if (settings.purposeId == null) {
				$('[name="purposeId"]', $popupModal).select2ajax({allowClear: true,placeholder: '',width: "100%"});
			}
