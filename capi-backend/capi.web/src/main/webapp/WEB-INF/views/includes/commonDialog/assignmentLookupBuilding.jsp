...
<script>
(function ($) {
	$.fn.assignmentLookupBuilding = function(opts) {
		var $input = this;
		var _def = {
			multiple: true,
			popupUrl: "<c:url value='/commonDialog/AssignmentLookup/buildingHome'/>",
			queryUrl: "<c:url value='/commonDialog/AssignmentLookup/buildingQuery'/>",
		...
		function initPopup() {
			...
			$popupTable.DataTable({
	            ...
	            "ajax": {
	            	...
	            	data: function(dataModel) {
	            		...
	            		dataModel.survey = $('[name="survey"]', $popupModal).val();
	            		dataModel.referenceNo = $('[name="referenceNo"]', $popupModal).val();
	            		...
	            		if (settings.survey != null)
	            			dataModel.survey = settings.survey;
	            		else
	            			dataModel.survey = $('[name="survey"]', $popupModal).val();
	            		settings.queryDataCallback(dataModel);
	            		
	            		if (settings.surveyMonth != null)
	            			dataModel.surveyMonth = settings.surveyMonth;
	            		else
	            			dataModel.surveyMonth = $('[name="surveyMonthId"]', $popupModal).val();
			...
			$('[name="survey"]', $popupModal).select2ajax({
				width:'100%',
				closeOnSelect: false
			});
			$('[name="surveyMonthId"]', $popupModal).select2ajax({
				width:'100%',
				closeOnSelect: false
			});
