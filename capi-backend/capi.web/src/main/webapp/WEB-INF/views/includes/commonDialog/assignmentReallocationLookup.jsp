...
<script>
(function ($) {
	$.fn.assignmentReallocationLookup = function(opts) {
		var $input = this;
		var _def = {
			multiple: true,
			popupUrl: "<c:url value='/commonDialog/AssignmentReallocationLookup/home'/>",
			queryUrl: "<c:url value='/commonDialog/AssignmentReallocationLookup/query'/>",
			selectAllUrl: "<c:url value='/commonDialog/AssignmentReallocationLookup/getLookupTableSelectAll'/>",
			defaultOrder: [[ 1, "asc" ]],
		...
		function initPopup() {
				...
				buttons.push({
								...
								"action": function( nButton, oConfig, flash ) {
									var dataModel = {
											...
											assignmentStatus: $('[name="assignmentStatus"]', $popupModal).val(),
											surveyMonthId : $('[name="surveyMonthId"]', $popupModal).val()
									};
			...
			$popupTable.DataTable({
	            ...
	            "ajax": {
	            	...
	            	data: function(dataModel) {
	            		...
	            		if (settings.assignmentStatus != null)
	            			dataModel.assignmentStatus = settings.assignmentStatus;
	            		else
	            			dataModel.assignmentStatus = $('[name="assignmentStatus"]', $popupModal).val();
	            		
	            		if (settings.surveyMonthId != null)
	            			dataModel.surveyMonthId = settings.surveyMonthId;
	            		else
	            			dataModel.surveyMonthId = $('[name="surveyMonthId"]', $popupModal).val();
			...
			if (settings.surveyMonthId == null) {
				$('[name="surveyMonthId"]', $popupModal).select2ajax({allowClear: true,placeholder: '',width: "100%"});
			}
