		...
		function deleteRecordsWithConfirm(data) {
			bootbox.confirm({
				...
				callback: function(result){
					if (result){
						$.post("<c:url value='/userAccountManagement/RankMaintenance/delete'/>",
							data,
							function(response) {
			...
			*/
			...
			$(document).ready(function(){
				...
				$dataTable.DataTable({
					...
					"buttons": [
						{
							...
							"action": function( nButton, oConfig, flash ) {
								window.location = "<c:url value='/userAccountManagement/RankMaintenance/edit?act=add'/>";
							}
						},
						{
							"text": "Delete",
					...
					"processing": true,
	                "serverSide": true,
	                //"ajax": "<c:url value='/userAccountManagement/RankMaintenance/query'/>",
	                "ajax": {
	                	url: "<c:url value='/userAccountManagement/RankMaintenance/query'/>",
	                	method: "post"
	                },
	                "columns": [
								{
	                            ...
	                            },
	                            {
	                            	...
	                            	"searchable": false,
	                            	"render" : function(data, type, full, meta){
	                            		var html = "<a href='<c:url value='/userAccountManagement/RankMaintenance/edit?act=edit&id='/>"+data+"' class='table-btn'><span class='glyphicon glyphicon-pencil' aria-hidden='true'></span></a>";
	                            		<sec:authorize access="hasPermission(#user, 512)">
