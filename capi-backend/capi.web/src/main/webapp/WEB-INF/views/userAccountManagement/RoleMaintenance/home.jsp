		...
		<script>
			function deleteRecordsWithConfirm(data) {
				bootbox.confirm({
					...
					callback: function(result){
						if (result){
							$.post("<c:url value='/userAccountManagement/RoleMaintenance/delete'/>",
								data,
								function(response) {
			...
			$(document).ready(function(){
				...
				$dataTable.DataTable({
					...
					"buttons": [
						{
							...
							"action": function( nButton, oConfig, flash ) {
								window.location = "<c:url value='/userAccountManagement/RoleMaintenance/edit?act=add'/>";
							}
						},
						{
							"text": "Delete",
					...
					"processing": true,
	                "serverSide": true,
	                //"ajax": "<c:url value='/userAccountManagement/RoleMaintenance/query'/>",
	                "ajax": {
	                	url: "<c:url value='/userAccountManagement/RoleMaintenance/query'/>",
	                	method: "post"
	                },
	                "columns": [
								{
	                            ...
	                            },
	                            {
	                            	...
	                            	"searchable": false,
	                            	"render" : function(data, type, full, meta){
	                            		var html = "<a href='<c:url value='/userAccountManagement/RoleMaintenance/edit?act=edit&id='/>"+data+"' class='table-btn'><span class='glyphicon glyphicon-pencil' aria-hidden='true'></span></a>";
	                            		<sec:authorize access="hasPermission(#user, 512)">
	...
	<jsp:body>
        ...
        <section class="content">
        	<div class="row">
				<div class="col-md-12" >
				<!-- content -->
					<form action="<c:url value="/userAccountManagement/RoleMaintenance/delete" />" id="dataForm" method="post">
