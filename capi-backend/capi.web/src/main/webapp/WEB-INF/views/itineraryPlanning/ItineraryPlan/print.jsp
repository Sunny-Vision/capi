...
<t:layout>
	...
	<jsp:attribute name="header">
		...
		<script type="text/javascript" src="https://maps.googleapis.com/maps/api/js?key=${googleBrowserKey}&language=zh-tw&signed_in=true"></script>
		...
		<script>
			...
			function drawRoute(directionsDisplay, locations, color) {
				...
				if (locations.length > 1) {
					...
					routeMakerList.push(createMarker(map, assignment, assignment.position, 1, color));
				}
			}
			...
			function getNewLocationsSequence(data, session) {
				...
				for(var i = 0; i < data.length; i++) {
					...
					if (data[i] != "") {
						...
						if (majorLocation.isFreeEntryTask == false && majorLocation.session != "" && majorLocation.session != null) {
				...
				for (var i=0 ; i < majorLocations.length ; i++) {
					...
					if (majorLocation.isNewRecruitmentTask == true) {
						...
						for (var j=0 ; j < majorLocation.itineraryPlanOutlets.length ; j++) {
							...
							$("#majorLocationTable_"+majorLocation.session+majorLocation.sequence+" tbody")
							.append($("#outletTemplate").tmpl(majorLocation.itineraryPlanOutlets[j]));
						}
					}
				}
		...
		<script id="taskTemplate" type="text/x-jquery-tmpl">
			<ul id="task_\${sequence}" class=" list-group sortableItem" majorlocationIndex="\${sequence}">
				<li class="list-group-item">
					...
					<div class="row">
						<div class="col-xs-1"><label>Location: </label></div>
						<div class="col-xs-11"> \${location} </div>
					</div>
				</li>
			</ul>
		</script>
	...
	<jsp:body>
		<section class="content-header">
        	...
        	<c:if test="${not empty model.itineraryPlanId}">
				<div class=" form-horizontal breadcrumb" style="width:240px">
					<div class="form-group" style="margin-bottom:0px">
			        	<div class="col-xs-5">Created Date:</div>
			        	<div class="col-xs-7"><c:out value="${commonService.formatDateTime(model.createdDate)}" /></div>
			        </div>
			        <div class="form-group" style="margin-bottom:0px">
			         	<div class="col-xs-5">Last Modified:</div>
			         	<div class="col-xs-7"><c:out value="${commonService.formatDateTime(model.modifiedDate)}" /></div>
			         </div>
		      	</div>
	      	</c:if>
        </section>
	       					...
	       					<div class="box-body">
	       						<div class="form-horizontal">
	       							<div class="form-group">
	       								<div class="col-xs-3"><label>Date:&nbsp;</label><c:out value="${commonService.formatDate(model.date)}" /></div>
	       								<div class="col-xs-4"><label>Field Officer:&nbsp;</label><c:out value="${user.staffCode}" />-<c:out value="${user.englishName}" /></div>
	       								<div class="col-xs-4"><label>Working Section:&nbsp;</label><c:out value="${model.session}" /> Session</div>
       								</div>
       								<div class="form-group">
  									...
  									<div class="form-group">
										...
										<div class="col-xs-12" style="height: 500px;" >
       							   	...
       							   	<div class="col-md-12" id="sessions">
       							   		<label id="labelSession_1" class="control-label" <c:if test="${model.session eq 'PE' }">style="display:none"</c:if>>A Session:</label>
       							   		...
       							   		<label id="labelSession_3" class="control-label" <c:if test="${model.session eq 'AP' }">style="display:none"</c:if>>E Session:</label>
